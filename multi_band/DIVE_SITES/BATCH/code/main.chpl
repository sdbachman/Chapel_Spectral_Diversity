...
proc convolve_and_calculate(Array: [] real(32), Mask: [] real(32), dive_site : [1..2] int(16), dswr_D : ?, B : ?, Output: [] real(64), locL:?, locC:?, locR :?, Mask_Size : real(32), t: stopwatch) : [] {
  ...
  forall (i,j,k) in zip(left_edge_row, left_edge_col, right_edge_col) {
    ...
    calc_distance(Array, Mask, locL, locL, tmpLL, bs, be, i, j);
    calc_distance(Array, Mask, locL, locC, tmpLC, bs, be, i, j);
    calc_distance(Array, Mask, locL, locR, tmpLR, bs, be, i, j);
    calc_distance(Array, Mask, locC, locC, tmpCC, bs, be, i, j);
    calc_distance(Array, Mask, locC, locR, tmpCR, bs, be, i, j);
    calc_distance(Array, Mask, locR, locR, tmpRR, bs, be, i, j);
    ...
    for l in (j+1)..k do {
      ...
      calc_distance(Array, Mask, locL, locL, tmpLL, bs, be, i, l);
      calc_distance(Array, Mask, locL, locC, tmpLC, bs, be, i, l);
      calc_distance(Array, Mask, locL, locR, tmpLR, bs, be, i, l);
      calc_distance(Array, Mask, locC, locR, tmpCR, bs, be, i, l);
      calc_distance(Array, Mask, locR, locR, tmpRR, bs, be, i, l);
...
proc main(args: [] string) {
  ...
  coforall loc in Locales do on loc {
    ...
    const locMaskSpace = MaskSpace;
    var locImage : [locImageSpace] real(32);
    var locMask : [locMaskSpace] real(32);
    var OutputArray : [0..<rows, 0..<cols] real(64);

    // Read in array
    var ff = open(in_name + ".bin", iomode.r);
    var rr = ff.reader(kind=ionative);
    ...
    // Read in mask
    var f = open(in_name + "_mask.bin", iomode.r);
    var r = f.reader(kind=ionative);

    for i in locMaskSpace.dim[0] {
      for j in locMaskSpace.dim[1] {
        var tmp : real(32);
        r.readBinary(tmp);
        locMask[i,j] = tmp;
      }
    ...
    for i in D.localSubdomain() {
      ...
      convolve_and_calculate(locImage, locMask, dive_site, dswr_D, B, OutputArray,locLeftMaskDomain, locCenterMaskDomain, locRightMaskDomain,Mask_Size, t2);
      t2.stop();
